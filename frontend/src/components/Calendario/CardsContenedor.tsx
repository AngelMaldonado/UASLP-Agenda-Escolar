...
export function CardsContenedor() {
  ...
  const eventos = useContext(AgendaContext).data.eventos?.filter(e =>
    e.fecha_inicio!.getMonth()! <= mes! && e.fecha_fin!.getMonth()! >= mes!
    && e.fecha_fin?.getFullYear() == año && e.tipo != TipoEventoEnum.ALUMNADO
  )

  return (
    <Stack gap={5} className="z-0">
      {eventos?.map((evento) => (
        <CardCalendarioContenedor key={"Card calendario" + evento.nombre + "-" + evento.id} evento={evento}/>
      ))}
    </Stack>
  )
}
